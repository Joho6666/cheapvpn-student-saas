...
`server/index.js` owns production startup, signal handling and background jobs. `server/app.js` owns the Express application and keeps API compatibility. Environment parsing is in `server/config/env.js`; SQLite schema, migrations and the open-order preflight are in `server/db/database.js`. `GenericSubscriptionProvider` handles safe supplier reads, while `MockProvider` is reserved for isolated tests. Native WeChat/Alipay QR payments live in `server/payments/` and confirm money with `completePayment()` before `activatePaidOrder()`. Generic HMAC callbacks remain supported. Cloudflare Worker is not the production payment source of truth.

SQLite is the source of truth for users, orders, payments, payment events, sessions, subscriptions, usage snapshots, referrals and support tickets. In-memory session/rate-limit caches are performance helpers only; authenticated sessions survive an API restart because their hashes are persisted.
